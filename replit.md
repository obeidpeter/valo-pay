...
- `VALOPAY_RUN_INTEGRATION=1 pnpm run test:integration` — the repository and scheduled-close suites against `DATABASE_URL`, which must be a disposable development database with the schema pushed, under any name; it runs every suite and names the failures at the end. The two migration rehearsals need a loopback PostgreSQL whose login can create databases and skip, saying why, elsewhere (such as Replit's managed database); the pull-request workflow runs them all against its own PostgreSQL service container.
