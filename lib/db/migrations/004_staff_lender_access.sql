...
-- Apply after 003_pilot_workflow.sql. It is repeatable. Creating the table
-- locks the membership and lender tables, and creating its index locks it
-- even when the index exists, so an open write on one of those tables holds
-- it back, and writes that come after it wait behind it. The lock wait is
-- limited to 5 s and each statement to 60 s: if either limit is reached
-- nothing changes, and it can be run again at a quieter moment.
BEGIN;
