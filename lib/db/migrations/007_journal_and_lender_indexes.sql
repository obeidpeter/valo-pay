...
-- owner, before deploying the build whose Drizzle schema declares them; until
-- they exist the readiness check stays ready but reports indexes_missing. Run
-- it once for each schema holding the application's tables, including each
-- isolated runtime schema, with that schema first on the search path
-- (PGOPTIONS='-c search_path=<schema>'). Never run at startup.
--
...
BEGIN
 ...
 LOOP
   -- The table the statements above reached along the search path, which is
   -- not always its first schema; an index lives in its table's schema.
   SELECT n.nspname INTO table_schema FROM pg_class t JOIN pg_namespace n ON n.oid = t.relnamespace WHERE t.oid = to_regclass(wanted.table_name);
   SELECT i.indisvalid AS valid, i.indisunique AS is_unique,
     ...
     WHERE i.indrelid = to_regclass(wanted.table_name) AND c.relname = wanted.index_name;
   IF NOT FOUND OR NOT existing.valid OR existing.is_unique OR existing.definition <> wanted.definition THEN
     RAISE EXCEPTION 'Index %.% is not the reviewed definition (% on %). Nothing was changed; review the existing index first.', table_schema, wanted.index_name, wanted.definition, wanted.table_name;
   END IF;
 END LOOP;
END
$verify$;
