...
export function buildAlerts(state: DomainState, now: string, audit?: AuditVerification | null): Alert[] {
  ...
  if (aged.length > threshold) alerts.push({ key: "unallocated_over_threshold", severity: "high", title: "Unallocated Payments over the threshold", detail: `${counted(aged.length, "Payment has", "Payments have")} been unallocated for more than 24 hours; the merchant threshold is ${threshold}.`, count: aged.length });
  const overdue = recordsOf(state, "exceptions").filter((item) => isOpenException(item.status) && Date.parse(String(item.data.dueBy)) < nowMs);
  if (overdue.length) alerts.push({ key: "exceptions_overdue", severity: "medium", title: "Exceptions past their deadline", detail: `${counted(overdue.length, "open exception is", "open exceptions are")} past the business-day deadline their type carries (EXC-02).`, count: overdue.length, linkedRecordId: overdue[0]!.id });
  const deferred = recordsOf(state, "exceptions").filter((item) => isOpenException(item.status) && item.data.type === "notice_not_evidenced");
  if (deferred.length) alerts.push({ key: "attempts_deferred", severity: "medium", title: "Attempts deferred for missing notice evidence", detail: `${counted(deferred.length, "planned attempt passed its", "planned attempts passed their")} notice deadline without provider acceptance (NOT-10).`, count: deferred.length, linkedRecordId: deferred[0]!.id });
