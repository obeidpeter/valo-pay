...
import { inWorkspace, verifyWorkspaceEncryption, protectWorkspacePayloads, runtimeIsolationVerified } from '../lib/valopay-store';
...
router.get('/v1/team/readiness',async(req,res)=>res.json(await inWorkspace(req,res,async ctx=>{
  // The database check is this transaction's own: a set that differs from the reviewed one refuses the request before this runs.
