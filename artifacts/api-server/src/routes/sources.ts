...
import { sourceProfileInputSchema, sourceBatchQualitySchema, paystackFixtureInputSchema, providerReplayInputSchema } from "@workspace/valopay-schema";
...
router.get("/v1/sources", async (req, res) => {
  const result = await withState(req, res, (state, ctx) => {
    ...
    return { ...sourceQuality(state, ctx.now), paystack: { mode: "test_only", externalConnectionVerified: false, canRunFixtures: ["Admin", "Operations", "Finance"].includes(ctx.role), state: "configuration_required", message: "A Paystack account, test credentials and an operator-provisioned connection are required for an external test. Local fixture results do not verify a Paystack connection.", events: events.slice(0,50).map(providerEventView), total: events.length, quarantined: events.filter(e => e.status === "quarantined").length, duplicates: events.reduce((sum,e) => sum + Math.max(0, Number(e.data.deliveryCount || 0) - 1), 0) } };
