...
/** A late pool checkout is returned unused: a timeout never starts a background transaction. */
function acquireClient(): Promise<PoolClient> {
  return new Promise((resolve, reject) => {
    let settled = false;
    const timer = setTimeout(() => { settled = true; reject(new Error('Export database connection timed out.')); }, 5000);
    pool.connect((error, client, release) => {
      clearTimeout(timer);
      if (settled) { release?.(); return; }
      settled = true;
      if (error || !client) reject(error || new Error('Export database connection unavailable.'));
      else resolve(client);
    });
  });
}
