...
/** The SECURITY DEFINER helpers migrations 005 and 006 install, each owned by the NOLOGIN helper role with a fixed search path. */
const runtimeIsolationHelpers = ["valopay_runtime_workspace", "valopay_runtime_admin", "valopay_runtime_writer", "valopay_runtime_lender", "valopay_runtime_lenders", "valopay_runtime_member", "valopay_runtime_invited", "valopay_runtime_clear_invitee_grants", "valopay_runtime_guard_workspace"];
...
export async function bindRuntimeIdentity(client: PoolClient, identity: { organizationId: string; userId: string }, invitation?: { token: string; verifiedEmails: string[] }) {
  ...
  const helpers = (await client.query<{ proname: string; safe: boolean }>(`SELECT p.proname,(p.prosecdef AND r.rolbypassrls AND NOT r.rolcanlogin AND NOT r.rolsuper AND NOT r.rolcreaterole AND NOT pg_has_role(current_user,p.proowner,'MEMBER') AND p.proconfig=ARRAY[$3::text]) AS safe FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace JOIN pg_roles r ON r.oid=p.proowner WHERE n.nspname=$1 AND p.proname=ANY($2::text[])`, [config.schema, runtimeIsolationHelpers, `search_path=pg_catalog, ${config.schema}, pg_temp`])).rows;
  ...
  if (!helpers.some(helper => helper.proname === "valopay_runtime_lenders")) unavailable("The isolated database still checks lender access row by row. Apply lib/db/migrations/006_runtime_isolation_scope.sql before enabling staff access.");
  if (helpers.length !== runtimeIsolationHelpers.length || helpers.some(helper => !helper.safe)) unavailable("The isolation helper owner or fixed configuration is unsafe.");
