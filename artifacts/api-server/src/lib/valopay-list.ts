...
export function pageRecords(records: ValopayRecord[], query: ListQuery): { items: ValopayRecord[]; total: number; nextOffset?: number } {
  ...
  if (query.search) { const search = query.search.toLowerCase(); items = items.filter((record) => `${record.name} ${record.reference} ${record.status} ${JSON.stringify(record.data)}`.toLowerCase().includes(search)); }
