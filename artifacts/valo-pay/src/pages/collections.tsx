...
export default function CollectionsPage() {
  ...
  const nextAction = (item: typeof instalments[number] | undefined) => {
    if (!item) return 'Link this attempt to an instalment';
    if (!isUnpaid(item.status)) return 'No collection action needed';
    if (item.status === 'in_dispute') return 'Review the customer dispute';
    if (item.status === 'unpaid_final') return 'Agree a next step with the lender';
    if (mandates?.items.find(mandate => mandate.id === item.data?.mandateId)?.status === 'pending_activation') return 'Follow up on mandate activation';
    if (latestFailure.has(item.id)) return 'Review the failed attempt and retry policy';
    if (item.status === 'partially_paid') return 'Review the remaining amount';
    return ownerOf(item) === 'valopay' || ownerOf(item) === 'valo' ? 'Check the collection schedule' : 'Follow up with the collection owner';
  };
              ...
              <table className="w-full min-w-[1120px] text-sm text-left">
                ...
                <tbody className="divide-y">
                  {isLoadingDue || isLoadingAttempts ? (
                    ...
                    displayed.map(({ key, item, attempt }) => (
                      <tr key={key} className="hover:bg-secondary/10">
                        ...
                        <td className="max-w-56 px-4 py-3 text-xs leading-relaxed">{nextAction(item)}</td>
