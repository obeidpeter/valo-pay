...
export default function CollectionsPage() {
  ...
  const handleAction = (item: any, action: string) => {
    if (action === 'backtest_policy') {
      ...
      if (!policy) {
        setImportResult({ valid: 0, invalid: 1, rows: [{ row: 0, status: 'invalid', message: 'No policy is available for this instalment. Check its linked mandate and policy.' }] });
        return;
      }
        ...
        <div className="xl:col-span-1 space-y-6">
          <section className="bg-card border rounded-xl shadow-sm p-5">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Paste synthetic records in CSV format. Download a sample to see the required columns. Do not use real customer data.
            </p>
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1" onClick={handleCommit} disabled={doImport.isPending || !importText || !importResult || importResult.valid === 0 || importResult.invalid > 0 || previewSignature !== `${importKind}:${importText}`} busy={doImport.isPending && Boolean(doImport.variables?.data.commit)} busyLabel="Importing data…">Import data</Button>
            </div>
            ...
            <ScrollFrame label="Instalments" className="flex-1 overflow-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y">
                  {isLoadingDue ? (
                    <LoadingRow colSpan={4} what="instalments" />
                  ) : !dueItems || dueItems.items.length === 0 ? (
                    <EmptyRow colSpan={4} title="No instalments recorded">Import synthetic instalments using the sample CSV to see them here.</EmptyRow>
                  ) : (
                    dueItems.items.map(item => (
                      <tr key={item.id} className="hover:bg-secondary/10">
                        ...
                        <td className="px-4 py-3 text-right space-x-2">
                          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => handleAction(item, 'simulate_failure')}>Simulate failure</Button>
                          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => handleAction(item, 'backtest_policy')}>Test policy</Button>
                        </td>
                      </tr>
                    ))
                  )}
