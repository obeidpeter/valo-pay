...
import { ExportJobControl, exportKindTitle } from '@/components/export-job-control';
import { PilotError, PilotHeading, PilotPanel, pilotField } from '@/components/pilot-ui';
import { readableLabel } from '@/components/record-label';
import { Button } from '@/components/ui/button';
import { formatDate, formatNumber } from '@/lib/formatters';

/** A saved job's state in the words of the status filter. */
const statusLabels: Record<string, string> = { queued: 'Waiting', running: 'Preparing', ready: 'Completed', failed: 'Needs retry' };
...
function SavedExports() {
  ...
  const list = useListRecords('exports', query, { query: { enabled: !!merchantId, queryKey: getListRecordsQueryKey('exports', query), refetchInterval: 5000 } });
  ...
  const statusOptions = [['all','All exports'],...Object.entries(statusLabels)];
