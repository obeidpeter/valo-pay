  ...
  return <div className="space-y-6">
    ...
    {workspace?.role !== 'Admin' ? <p role="status" className="rounded-xl border bg-card p-5">Only a currently authorised administrator can inspect or change retention controls. Ask your administrator about holds and approved deletion runs.</p> : <>
      ...
      {data && <>
        ...
        {selected && <RunReview key={`${selected.id}:${selected.status}`} run={selected} locked={locked} submit={submit} secondApprover={data.secondApprover === true} actor={data.actor} />}
...
function RunReview({ run, locked, submit, secondApprover, actor }: { run: LifecycleRunView; locked: boolean; submit(path: string, body: unknown, response: 'run'): void; secondApprover: boolean; actor: string }) {
