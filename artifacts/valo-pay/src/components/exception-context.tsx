...
export function resolutionEffect(exception: ValopayRecord, code: unknown): string | undefined {
  ...
  if (waitingReversal(exception)) {
    ...
    if (!chosen) return 'Leave this exception open while you check with the provider which collection the reversal reverses: if its payment arrives meanwhile through the same connection, the reversal applies to it; through another connection, the reversal is held for you with an exception of its own. Either way this exception closes. Once the provider has answered, choose the outcome, and this box says what the next reconciliation does with it.';
