...
import { amountUnitName, canonicalJson, importFieldsOf, koboToNaira, suggestImportField } from '@workspace/valopay-schema';
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { useDialogFocusReturn } from '@/lib/focus';
import { formatCount, formatKobo, formatNumber } from '@/lib/formatters';
...
export function ImportWizard({ merchantId }: { merchantId: string }) {
  ...
  const doImport = useSafeImportRecords({ mutation: { onSuccess: (data, variables) => {
    ...
    if (variables.data.commit) {
      ...
      const submitted = variables.data.mapping as Record<string, string> || {}, headers = data.columns || Object.keys(submitted);
      // A header that is no destination is matched as its name suggests (full_name as the name) while no other column has that destination; otherwise it is skipped.
      const direct = (header: string) => Object.hasOwn(submitted, header) ? submitted[header]! : header === 'amount' ? 'amountKobo' : fields.includes(header) ? header : undefined;
      const taken = new Set(headers.map(direct).filter(Boolean));
      const chosen = Object.fromEntries(headers.map(header => {
        const target = direct(header), suggestion = target === undefined ? suggestImportField(variables.data.kind, header) : undefined;
        if (target !== undefined) return [header, target];
        if (!suggestion || !fields.includes(suggestion) || taken.has(suggestion)) return [header, ''];
