...
export function ExportJobControl({ kind, customerId, closeReviewId, formats = ['pdf'], label }: { kind: string; customerId?: string; closeReviewId?: string; formats?: Format[]; label: string }) {
  const { merchantId, workspace } = useWorkspace();
  const canCreate = !!workspace && workspace.role !== 'Read-only';
  const queryClient = useQueryClient();
  const scope = `${merchantId}:${kind}:${customerId || ''}:${closeReviewId || ''}`;
  ...
  const title = kind === 'billing' ? 'Billing CSV' : kind === 'gate-pack' ? 'Evidence pack' : kind === 'reviewed-close' ? 'Reviewed close evidence' : kind === 'closes' ? 'Close evidence' : 'Dispute pack';
  ...
  const previous = recent.data?.items.filter(record => record.data.kind === kind && (!customerId || record.customerId === customerId) && (!closeReviewId || record.data.closeReviewId === closeReviewId)) || [];
