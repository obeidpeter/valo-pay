...
/** A saved export's state in words, the one list Saved exports filters by. `expired` is no job status: the list derives it from a file an approved retention run removed, which Completed and Needs retry then leave out. */
export const exportStatusLabels: Record<string, string> = { queued: 'Waiting', running: 'Preparing', ready: 'Completed', failed: 'Needs retry', expired: 'File expired' };
...
export function ExportJobControl({ kind, customerId, closeReviewId, savedJobId, formats = ['pdf'], label }: { kind: string; customerId?: string; closeReviewId?: string; savedJobId?: string; formats?: Format[]; label: string }) {
  ...
  return <div className="space-y-3 min-w-0 max-w-xl">
    ...
    {!canOpen ? <p className="text-xs text-muted-foreground">{sensitiveExportRefusal} Ask one of them for this file.</p> : !canCreate && <p className="text-xs text-muted-foreground">Read-only access lets you download existing files. Ask a teammate with a working role to generate or retry an export.</p>}
    ...
    {job && <div role={state === 'failed' ? 'alert' : 'status'} className="rounded-lg border bg-card p-3 text-sm space-y-2">
      ...
      {state === 'expired' && <><p className="text-muted-foreground">Removed on {formatDate(job.expiredAt!)} by an approved retention run under the lender’s retention policy. {job.checksum ? '' : 'This export did not finish, so it has no checksum to keep. '}Create a new export for current evidence.</p>{job.checksum && <p className="font-mono text-xs break-all">SHA-256 of the removed file: {job.checksum}</p>}<p className="text-xs text-muted-foreground">Its deletion receipt is saved with the retention run that removed it.{job.retentionRunId && workspace?.role !== 'Admin' ? ' An administrator sees a link to it on this export.' : ''}</p>{job.retentionRunId && <p className="font-mono text-xs break-all">Retention run: {job.retentionRunId}</p>}{job.retentionRunId && workspace?.role === 'Admin' && <Link href={`/lifecycle?run=${encodeURIComponent(job.retentionRunId)}`} className="inline-flex min-h-9 items-center text-xs text-primary underline">Open its deletion receipt</Link>}</>}
