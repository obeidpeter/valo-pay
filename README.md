...
Each API process counts its own request limits (`artifacts/api-server/src/lib/request-limits.ts`); nothing is written to the database for them. A principal may make 300 requests a minute: a signed-in person, an anonymous sandbox the process has already served, and otherwise the client's network, which is an IPv4 address or an IPv6 /64, since one subscriber can use any address in its /64. A network may make 1,200 a minute whoever sends them, so rotating cookies or accounts cannot escape the limit, and colleagues behind one office or carrier address share that ceiling but not a quota. New anonymous sandboxes are limited to 20 an hour per network, 60 per IPv6 /48 and 300 per process; Paystack test deliveries to 120 a minute per network and 60 per connection. A refusal is a 429 with `Retry-After`. Because each process counts alone, a host that runs several instances multiplies every limit by the number of instances a client's requests reach, and a restart or a scale to zero starts every count afresh: the limits bound what one client costs one instance, and a limit across the whole deployment belongs to the host's edge. Every count is held in a map of bounded size that a timer sweeps, so no request pays for a large one.
