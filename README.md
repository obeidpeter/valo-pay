...
`GET /api/healthz` answers without a sandbox or a sign-in: the build, the start time, the uptime and what the close scheduler is doing. `GET /api/readyz` makes one bounded round trip to the database on its own connection, which also checks that every table, column and index this build needs is present, and answers 503 while either fails, naming anything missing and the migration that adds it; apply `lib/db/migrations/007_journal_and_lender_indexes.sql` before deploying this build (see `docs/record-list-index-deployment.md`).
