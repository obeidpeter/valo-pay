...
| `CLERK_JWT_KEY` | Optional, and recommended wherever Clerk is configured: the public key the Clerk instance signs session tokens with, in PEM form (Clerk lists it with the instance's API keys as the JWT public key). With it the API verifies every session token itself, with no network call. Without it a token that names a key Clerk has not cached, as any forged token can, makes the API fetch the instance's keys from Clerk's Backend API with the secret key, which uses up that API's rate limit; only the request limits of `/api/v1` bound those fetches |
