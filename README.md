...
| `VALOPAY_DATABASE_POOL_SIZE` | Optional; the database connections one API process may hold, a whole number from 2 to 100, default 10. One tenant (a sandbox or a staff organisation) may use at most a third of them for its lenders, and one of its lenders at most half of that (at least one each), so a busy tenant, even one with two busy lenders of its own, leaves the rest to everyone else. A staff host that serves one organisation therefore gives it at most a third of the pool for its lenders: size the pool for that (30 gives it 10, and 5 for one lender). Keep instances × size (plus one short-lived readiness connection per instance) within the database's connection limit. |
