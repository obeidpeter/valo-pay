...
The revision covers what the workspace shows and what its actions read, and nothing else: the lender and its settings, the customers, every connected record, each pay-by-bank receipt with its allocations, and the instalments Pay-by-bank offers, a checkout names or a receipt was applied to, with their attempts. A change to any of these, in another tab or by a colleague, makes an action sent from the older view answer 409. The lender's history (closes, the audit trail, exports, settled instalments and other payments) no longer does, and the revision no longer differs between the read that shows the view, which loads every close in full, and the write that checks it, which loads closes more than a week older than the latest as summaries: before, once a lender had such a close, every connected action answered 409. The view lists the lender's customers once, as `customers`; the Credit Desk's `credit.permissions` lists only the applicants holding a permission, so an applicant not listed holds neither.
