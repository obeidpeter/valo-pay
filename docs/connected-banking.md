...
4. Record a sample confirmed, failed or unknown provider outcome. An unknown result holds the instalment: no other checkout and no retry until the outcome is known, so query it before proceeding. Once the outcome has been unknown for 24 hours, the daily close raises an unknown-outcome exception owned by Finance, the overview shows an alert and the checkout links to the exception. Finance resolves it as confirmed successful, with the masked reference of the evidence that the money arrived (the receipt is then recorded and applied to the instalment), or as confirmed failed; either releases the instalment. A sample outcome recorded later closes the exception because its condition cleared, and the step's audit entry names the exceptions it closed, as a refund or reversal step's does.
...
6. For a refund, Admin/Operations records the request and a different Finance persona records sample refund evidence. A confirmed refund records the whole receipt as returned (data.refundedKobo) and reopens the instalment it paid by its balance, open for collection and not in dispute. Finance can also record reversal evidence: the instalment it paid owes that amount again and is in dispute, with a customer dispute exception for Operations. Resolving that exception as not upheld, or Finance's release from dispute on the Collections page, takes the instalment out of dispute. No refund or reversal sends money.
...
3. Change the downside receipt percentage, delay and planning buffer. Save with a review note to create a new forecast version. Committed outflows remain due: an approved outflow past its due date is still owed and counts as due now, in every week of both scenarios, while a receipt past its due date is left out until it arrives. Reserves are planning assumptions and do not alter bank balances.
...
5. In **VAT evidence**, compare approved invoice tax amounts with payment allocations and the ledger control. The sample tax period is the West Africa Time month, so a sample set up at 00:30 WAT on the 1st belongs to that month. Loan proceeds and own-account transfers do not become taxable sales. Purchase VAT without an approved recovery decision remains excluded. Finance can save a review schedule with its unresolved gaps; downloading it does not file or pay tax.
...
The revision covers what the workspace shows and what its actions read, and nothing else: the lender and the one setting the workspace reads (its environment), the customers, every connected record, each pay-by-bank receipt with its allocations, and the instalments Pay-by-bank offers, a checkout names or a receipt was applied to, with their attempts. A change to any of these, in another tab or by a colleague, makes an action sent from the older view answer 409. The lender's history (closes, the audit trail, exports, settled instalments and other payments) and its other settings (the scheduled close's cursor and retry, a pause for inactivity and the audit chain's head, which every audited write moves) no longer do, so a scheduled close that changes nothing the workspace shows leaves a reviewed form current, and the revision no longer differs between the read that shows the view, which loads every close in full, and the write that checks it, which loads closes more than a week older than the latest as summaries: before, once a lender had such a close, every connected action answered 409. The view lists the lender's customers once, as `customers`; the Credit Desk's `credit.permissions` lists only the applicants holding a permission, so an applicant not listed holds neither.

The view and every action's answer are described by the shared schemas in `lib/valopay-schema/src/connected.ts`, from which the contract's `ConnectedWorkspace`, `ConnectedActionResult` and their parts are generated. The API checks each answer against them before it commits, and the console reads the answers with the same schemas (`artifacts/valo-pay/src/lib/connected.ts`), so a malformed answer is shown as incomplete, and an action's as an unconfirmed outcome, never as data. An action's answer is checked, by both, in the one shape its action gives (`connectedActionResultFor`): a `cash.*` action answers its outcome, with the Cash Desk record it saved or changed and, for an export, its manifest; every other action answers the consent, checkout, assessment or review it produced, of the lender the request named. The contract's `ConnectedActionResult` accepts either shape, because an answer does not say which action it answers.
