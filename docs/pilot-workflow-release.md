...
Apply `lib/db/migrations/003_pilot_workflow.sql` with the existing migration procedure before deploying this API. It adds five tables without rewriting existing financial records. It is repeatable and transactional. Back up the synthetic development database first. Rollback means restoring the previous application build and leaving the additive tables in place; do not drop the journal or access history during rollback.
