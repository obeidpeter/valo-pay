...
003, 004 and 007 are single transactions and repeatable: run again, they change nothing. Use a direct or session-pinned connection for 002, whose runner holds an advisory lock for the whole operation. Keep `DATABASE_URL` in the environment rather than on the command line, and never paste its value into a terminal log, an issue or a report.
