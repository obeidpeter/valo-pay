...
The catalogue for the numbered files in `lib/db/migrations`: which a database needs, who applies each, how to verify it and how to roll back. Nothing applies them automatically: not installation, the build, start-up, the post-merge hook or a deployment. Numbering does not mean every file belongs in the same database.

- **Application schema:** 002, 003, 004, 007, 008 and 009 are changes for existing application databases. The database owner applies the required changes in order before publishing the build that needs them. File 001 is superseded and must not be applied.
- **Optional restricted runtime:** 005 and 006 commission an isolated runtime schema only when that mode is explicitly selected. They are not ordinary application-schema upgrades.
- **Optional financial staging:** 010 installs a separate synthetic financial projection for a reviewed rehearsal. Existing v1 records remain authoritative; ordinary deployment does not need it.
- **Disposable recovery rehearsal only:** 011 and 012 belong in two distinct local test databases, not in production or the application's normal schema. The rehearsal runner installs and removes its own isolated databases.

## Where the Publish flow fits

Replit's Publish flow deploys a build and carries the development database's schema across to the production database once its diff has been reviewed; it never runs these files. A fresh development or CI database gets the ordinary application tables, constraints and indexes from the Drizzle schema with `pnpm --filter @workspace/db run push`; that does not install the optional isolation policies or the separate objects in 010–012. An existing application database (production, a staging host, a restored backup) gets each release's required application changes from the files below, applied with the owner's connection before the release is published. Then review the Publish flow's diff as `docs/DATABASE_SECURITY.md` requires: once the required files are applied it shows no change for their application tables and indexes, and any unexpected drop or rename stops the release. Do not carry rehearsal schemas into production through that diff.
...
## File catalogue and applicability
...
| 010 | `lib/db/migrations/010_financial_projection_staging.sql` | Optional synthetic staging only, in a new `valopay_finance_staging_<suffix>` schema. Not a production prerequisite. | The migration owner of an isolated rehearsal database. | Use the initialisation and bounded backfill commands in the [financial staging runbook](financial-projection-staging.md). | Verify the schema fingerprint and exact source parity; backfill rolls back unless `--apply` is selected. Ordinary readiness does not certify this projection. | Disable staging dual writes and restart the staging app. Keep v1 records; retain or remove only the isolated projection after an owner review. |
| 011 | `lib/db/migrations/011_synthetic_instruction_recovery.sql` | Disposable synthetic rehearsal only: commands, authority, queue and inbox in the test application database. | The rehearsal runner, using an authorised local test login. | `pnpm run rehearse:instructions` with the [recovery runbook](synthetic-instruction-recovery.md) prerequisites. | The suite verifies refusal guards, identity, claims, interruption and recovery against a separate journal. Not covered by application readiness. | The suite removes its own generated test database; there is no application migration to reverse. |
| 012 | `lib/db/migrations/012_synthetic_independent_journal.sql` | Disposable synthetic rehearsal only: dispatch intent and synthetic receipts in a distinct journal database. Never apply it to the application database. | The same rehearsal runner. | The same `pnpm run rehearse:instructions` run installs 012 in its journal database. | The suite verifies independent intent, append-only guards and recovery after test application-schema loss. Not proof of separately managed production custody. | The suite removes its own generated journal database after the rehearsal; preserve the test report. |
...
## Before and after an application-schema change
...
3. Apply only the application-schema files the release needs, in order, with the commands above. Optional commissioning and disposable rehearsals follow their separate runbooks; do not add 010–012 to a production migration sequence.
...
6. Review the Publish flow's schema diff before confirming deployment, then publish the build. Check readiness again on the deployed revision.
...
3. Apply 007, 008 and 009 in that order in the runtime schema too: `PGOPTIONS="-c search_path=$VALOPAY_RUNTIME_SCHEMA" psql "$MIGRATION_OWNER_URL" -X -v ON_ERROR_STOP=1 -f lib/db/migrations/007_journal_and_lender_indexes.sql`, then the same command with `lib/db/migrations/008_export_queue_index_and_foreign_key_names.sql` and `lib/db/migrations/009_record_identity_guards.sql`. The copied tables have no foreign keys, so 008 adds only the index there. Follow the [009 preflight](record-identity-migration.md) for any existing runtime records; never remove evidence to force a guard to install.
...
## Separate financial and recovery rehearsals (010–012)

Migration 010 requires both a schema matching `valopay_finance_staging_<suffix>` (a suffix of 1–32 lowercase letters, digits or underscores) and the SQL setting `valopay.financial_migration=staging-only`. The financial operator command sets that SQL opt-in after requiring `VALOPAY_FINANCIAL_PROJECTION=staging`; initialisation refuses an existing schema. It installs the projection in a transaction, although a failed installation may leave the newly created empty schema for the owner to inspect. Follow the [financial staging runbook](financial-projection-staging.md) for source bounds, review, explicit commit and rollback. With staging disabled, the API needs none of these tables. With it enabled, each paired write verifies the projection's fingerprint and parity and refuses the whole transaction on failure; a healthy `/api/readyz` result alone does not verify it.

Migrations 011 and 012 each require `valopay.instruction_migration=synthetic-only` and a `valopay_dispatch_staging_<suffix>` schema. These SQL guards alone do not establish database separation: the [synthetic recovery runner](synthetic-instruction-recovery.md) additionally requires an opted-in loopback test server, creates distinct `valopay_instruction_app_*` and `valopay_instruction_journal_*` databases, and installs each file in a transaction in its intended database. Neither file is a repeatable application upgrade; let the runner create fresh isolated objects. Its normal cleanup removes only its own generated databases. After an interrupted run, inspect the exact generated names before any manual cleanup; never remove an application database or schema. No provider is contacted, and passing the rehearsal does not enable a deployment feature.
