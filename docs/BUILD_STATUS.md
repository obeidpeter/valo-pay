...
Pilot security work is isolated from the running sandbox. A separate staging app connects verified-session access/MFA, fresh provisioned membership, a restricted database login, forced row-level isolation and encrypted synthetic-note persistence with atomic audit/replay records. Disposable tests exercise HTTP requests through this path. A real PostgreSQL backup/restore rehearsal records timings and verifies records, balances, audit chains, replay responses and synthetic key/export fixtures. A one-shot external monitor supports incident/recovery delivery to the selected operator. These are rehearsed components, not activated production controls: the host has no outbound email service, separate staging identity app or managed key provider configured. Full pilot workflows and production recovery still need deployment-specific integration and acceptance. See `docs/operational-rehearsals.md`, `docs/pilot-security.md` and `docs/pilot-database.md`.
