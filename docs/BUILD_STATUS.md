...
- An alerts feed on the overview and in every daily close for the NFR-OBS-02 conditions the sandbox can observe: a broken audit chain, an instruction in observation mode, position drift, unallocated Payments over the merchant threshold, overdue exceptions, deferred attempts, notification cost per collection and an overdue close.
