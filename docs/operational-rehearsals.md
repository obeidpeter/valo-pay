...
Run `pnpm run check:operations` with `VALOPAY_MONITOR_ORIGIN` set to the HTTPS service origin. The default is a dry run: it prints only the service origin, check time and fixed failure codes. It reads no customer records and sends no alerts. The one option is `--deliver`; a `--` before it, as `npm run` needs, is skipped. A mistyped option is named in the error with the usage; a word that is not an option is only counted, in case it is a credential. Any other failure prints one general message, so no credential or response body reaches the terminal.
