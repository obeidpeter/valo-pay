...
Candidate branch: **codex/evidence-led-usability**. Review as a **draft pull request**. This assignment authorises the existing synthetic preview only. Production publishing/merging, live providers, real-data use, customer communications and paid resources are outside this release. No new schema, secret, feature flag or dependency is needed. Keep scheduler/external side-effect configuration as found.

The code includes a bounded backend correction for proposal identity, allocation ceilings and instalment eligibility because a UI explanation cannot contain those defects. The corresponding tests are required alongside the frontend. Obtain an independent qualified engineer's review before merging this sensitive change, consistent with existing NFR-OPS-06; a passing automated suite is not that review.
...
- **CSV:** choose the source amount unit, match each destination once, check all rows and inspect errors. Checks save nothing. The current contract is atomic for new rows; duplicates with a recognised non-empty reference are skipped. Keep references stable. A new reference-free import can create another record.
...
- **Unknown result:** keep the page/dialog open and use its original-request retry. Inputs are held to prevent a second intention. A later session/permission rejection does not establish whether the first request committed. Closing/reloading may lose local retry information; inspect saved records, jobs and audit history before starting again. Do not resend merely because a toast or connection disappeared.
- **Exports:** queued/running jobs can be resumed through saved history. A failed job retry refers to that saved job; an uncertain creation retry refers to the original request. Download access is checked again. No expiry date is advertised because the API provides none; a downloaded copy cannot be recalled.
...
Local verification completed: **320 UI tests across 61 files**, all frontend/API/project and test TypeScript checks, frontend and API builds, database source boundary and documentation checks. Reconciliation passes **414** golden checks and the adjacent daily-close suite passes **110**. The wider available offline backend, security, privacy, import, export, retry, billing, recovery and performance checks pass. The Linux-only snapshot symlink test cannot run as written on Windows; the unchanged canonical suite runs in CI. No PostgreSQL instance or non-Chromium browsers were assumed locally.

The complete desktop/phone Chrome workflow suite passed **36 tests** after the proposal fixture and owner-filter correction. The final additional response validation and replay guidance have their own focused regressions and the full 320-test UI pass; CI must validate the final built head in all configured browsers. The local isolated scroll reproduction returns **1800 → 0 → 1800** across tall queue, short page and return. Both builds retain the existing non-blocking main-chunk size warning; the frontend build also reports the existing sheet source-map warning.

CI, candidate identity and actual preview smoke results belong to the linked draft PR and delivered evidence manifest. Do not infer deployment from this local record. The new real-API/PostgreSQL browser test commits a synthetic customer, deliberately withholds and loses its response, checks pending/unknown controls, then proves identical-key/body replay and exactly one saved customer.

## Before and after interaction paths

| Journey | Baseline friction | Candidate path and authoritative outcome |
| --- | --- | --- |
| Import | Preview and commit completion looked similar; duplicate-only files offered an unproductive import; errors were harder to revisit | Map once → check without saving → filter/correct invalid rows → import all valid new rows once; duplicate-only result is terminal and explicit |
...
1. Verify draft PR head, clean source tree, successful required CI and reviewed changed paths. Use the exact candidate source in the existing Replit development preview; do not press Publish or alter provider/scheduler configuration.
...
3. Keep the production build unchanged. Human/accessibility and independent-engineer review remain separate acceptance work. Publish only after a later explicit decision with these residuals visible.
...
| UX-AM02 | API-03, CON-04 / UX-C01, UX-R03–04 | Require reviewed proposal identity/version in a future compatible API revision; preserve unresolved outcomes through auth failures; define durable scoped recovery before live writes. Maintain tenant/amount/eligibility and replay invariants |
...
| UX-AM06 | TEN-01/02/03, UI-05 / UX-B02/05/06 | Before real data, resolve durable request lookup, authenticated user-scoped preferences, sensitive-search retention and fresh MFA/revocation tests. Do not expand demo personas into production authority |
