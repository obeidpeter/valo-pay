...
### UX-X06 — Unknown request outcomes could be replaced (resolved within the open form/session; durable recovery deferred)
...
- Effort/dependency: medium; existing server idempotency and root shared safe-mutation work. Residual: original retry identity is in-memory. A deliberate navigation discard/tab close/reload can lose the local recovery session; do not claim durable or cross-device replay. Operator audit/reconciliation and a separately scoped durable recovery design are needed for that case; no sensitive unrestricted storage was introduced.
...
| --- | --- | --- |
| UX-X07 | Deferred, severity 2 hypothesis | Durable unconfirmed-operation recovery after close/reload needs a scoped server status resource and identity design. Current fix intentionally keeps request bodies out of browser storage. Test server replay/tenant boundaries and operator recovery before any live acceptance. |
