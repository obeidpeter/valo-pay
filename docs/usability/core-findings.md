...
- Exports are saved queued/running/ready/failed jobs with per-download authorisation and idempotent retries. At this inspection the API had no public expiry field, and no expiry or recall feature was invented. Since 25 September 2026 an export whose file an approved retention run removed reports when (`expiredAt`) and by which run (`retentionRunId`), and Saved exports can list such files; by the owner's decision a ready export still shows no expiry date, since only an approved retention run removes a file and a hold can keep it longer. A downloaded copy still cannot be recalled.
...
Residual at this entry: the identity/version pair remained optional for older API callers, by explicit root decision to retain the current public action contract, while the console always supplied it. Closed on 25 September 2026 (UX-B01): both actions now require `data.proposalId` and `data.proposalUpdatedAt` and refuse a request without either with 400 naming the missing field, a breaking change for API clients made without a new API version, and the PostgreSQL suite `artifacts/api-server/tests/allocation-decisions.integration.test.ts` shows two concurrent decisions on one proposal applying once. A timestamp cannot detect an unchanged timestamp on externally altered data; normal persistence must maintain `updatedAt`. No new cross-customer allocation policy was invented for unidentified payer records.
...
Paths: `artifacts/valo-pay/src/components/export-job-control.tsx`, `artifacts/valo-pay/tests/export-jobs.test.tsx`, `tests/reports.test.tsx`, `tests/evidence.test.tsx`, `tests/toasts.test.tsx`. Residual: the page's own recovery memory is per mounted UI scope, so after a reload its retry key is gone. The service's journal keeps the export request it received: Operations checks it with its original body and key, or cancels it (UX-B02, decided: recovery stays manual, through Operations), and the export control's notice for an unconfirmed request says so and links there (`artifacts/valo-pay/tests/recovery-notices.test.tsx`). Sensitive request bodies are not persisted in browser storage. No expiry date is shown for a ready file.
...
- Current technology references consulted: [React state](https://react.dev/reference/react/useState), [TanStack Query mutations](https://tanstack.com/query/latest/docs/framework/react/guides/mutations). No new dependencies or API generation needed; the fields travelled in existing free-form action data. Since 25 September 2026 they are required, and the contract names them (`AllocationDecisionData`).
