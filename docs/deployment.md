...
Before changing this setting, capture `node scripts/commission-operations.mjs` with the intended scheduler mode and actual hosting type (`docs/operational-rehearsals.md`). A healthy API with `VALOPAY_CLOSE_SCHEDULER=off` proves no automatic close. A fresh in-process heartbeat on Autoscale does not prove the process will exist at the next due time. The commissioning report keeps these observations separate from the completed close, alert receipt and isolated recovery evidence required for acceptance.
...
Commission the scheduled job on an isolated synthetic host first: record the deployed build, due time and business date; let the job perform the close; confirm one scheduled close and its reviewed evidence; then test an interrupted or failed run and its recovery without duplicate closes. Configure an independent alarm for a missing or failed job execution, and verify that alarm reaches the selected operator. Keep run identifiers, expected/observed times and the operator's acknowledgement in the private commissioning record. Do not treat the web process's `external` mode or a successful configuration save as job completion evidence. A machine-readable report explicitly leaves external completion and missed-run detection unverified until that exercise has been observed.
...
The deployment sets `VALOPAY_EXPIRED_WORKSPACE_CLEANUP=on`. An anonymous sandbox becomes eligible for deletion when it is more than 30 days old and nobody has changed it for 30 days (entries by the seed and the scheduled close do not count as changes). Deletion happens only when a new anonymous sandbox is created, that is when a browser without a sandbox cookie first opens the console: that request deletes up to five eligible sandboxes, oldest first, removing their lenders, records, stored answers and operations journal. A sandbox whose lender a scheduled close, the export worker, a test delivery or a request holds at that moment is left for a later sweep. So an eligible sandbox is deleted at the next new visitor's arrival, five at a time, not at the moment it turns 30 days idle; on a quiet host it can stay longer. Once that deletion has committed, the same request removes from private storage the export files the swept sandboxes' lenders wrote there, each only if its metadata names that export and lender (a file an approved retention run removed is not touched again); it starts no removal after five seconds, so the new visitor is not held for long. A storage failure never undoes the deletion: a file private storage could not remove, or one not reached within the five seconds, is named in a `workspace.sweep_file_left` log line with its bucket and object name (`docs/observability.md`), and an operator removes it by hand, since nothing else refers to it any more. Signed-in and staff workspaces are never swept. The development workspace leaves the setting unset, so nothing is deleted there.
