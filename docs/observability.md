...
| `request.busy` | warn for `lender_busy`, `lock_timeout` and `lock_conflict`; error otherwise | `status` (503), `limit`, `reason`, `err` | The request was turned away at a database limit before anything was saved: `lender_busy` (the lender's share of connections was taken for 5 s), `lock_timeout`, `lock_conflict` (a deadlock or serialisation failure), `statement_timeout`, `idle_timeout`, `connection_lost`, `pool_timeout` (no free connection within 5 s) or `database_unavailable`. `err` is the database's own error. |
