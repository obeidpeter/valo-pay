...
A successful independent test verification records a normalised observation and verification history. It creates no payment or allocation itself; ordinary reconciliation consumes the observation under the existing rules, and separate settlement evidence is still required. Timeouts and unavailable responses retain an unknown outcome rather than authorising another debit. The operator command is explicit, not a scheduled poller.
